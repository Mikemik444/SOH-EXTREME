...
static bool IsQuestSkipped(uint8_t quest) {
    switch (quest) {
        ...
        case QUEST_ARCHIPELAGO:
            // Keep Archipelago visible at all times. Selection itself still requires
            // an authenticated SOH-EXTREME connection.
            return false;
        default:
            return false;
    }
...
void FileChoose_UpdateQuestMenu(GameState* thisx) {
    ...
    if (CHECK_BTN_ALL(input->press.button, BTN_A)) {
        uint8_t selectedQuest = this->questType[this->buttonIndex];

        // QUEST_ARCHIPELAGO only exists in file select. Save files use
        // QUEST_RANDOMIZER internally so all existing rando hooks/checks work.
        gSaveContext.ship.quest.id =
            (selectedQuest == QUEST_ARCHIPELAGO) ? QUEST_RANDOMIZER : selectedQuest;

        if (selectedQuest == QUEST_ARCHIPELAGO) {
            if (!Archipelago_IsAuthenticatedForFileSelect()) {
                Audio_PlaySfxGeneral(NA_SE_SY_FSEL_ERROR, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                     &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
                return;
            }
            ...
            this->logoAlpha = 0;
            return;
        } else if (selectedQuest == QUEST_BOSSRUSH) {
            Audio_PlaySfxGeneral(NA_SE_SY_FSEL_DECIDE_L, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                 &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            this->prevConfigMode = this->configMode;
            this->configMode = CM_ROTATE_TO_BOSS_RUSH_MENU;
            return;
        } else if (selectedQuest == QUEST_RANDOMIZER) {
            Audio_PlaySfxGeneral(NA_SE_SY_FSEL_DECIDE_L, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                 &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            this->prevConfigMode = this->configMode;
    ...
    if (this->configMode == CM_NAME_ENTRY_TO_QUEST_MENU || this->configMode == CM_BOSS_RUSH_TO_QUEST ||
        ...
        if (this->windowRot >= 314.0f) {
            if (CountVisibleQuests() > 1) {
                ...
                } else if (!IsQuestSkipped(QUEST_ARCHIPELAGO)) {
                    // Archipelago has no local settings menu; go directly to
                    // naming the file after authentication.
                    this->configMode = CM_START_NAME_ENTRY;
    ...
    if ((this->configMode == CM_QUEST_MENU) || (this->configMode == CM_START_QUEST_MENU) ||
        ...
        switch (this->questType[this->buttonIndex]) {
            ...
            case QUEST_ARCHIPELAGO: {
                ...
                // No new texture asset is required: draw the AP subtitle with
                // SoH's normal text renderer so this patch drops into existing
                // projects without regenerating OTR assets.
                char apQuestLabel[] = "ARCHIPELAGO";
                Interface_DrawTextLine(this->state.gfxCtx, apQuestLabel, 113, 180, 80, 220, 255,
                                       this->logoAlpha, 0.9f, true);
                break;
            }
        }
        ...
        for (i = 0; i < 3; i++, temp += 20) {
            ...
            // draw rando label. Archipelago saves get a clearer text badge below
            // instead of the generic RAND badge so players can distinguish the
            // server-backed file at a glance.
            if (Save_GetSaveMetaInfo(i)->randoSave && !Save_GetSaveMetaInfo(i)->archipelagoSave) {
                if (!FileChoose_IsSaveCompatible(Save_GetSaveMetaInfo(i))) {
                    gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, sWindowContentColors[1][0], sWindowContentColors[1][1],
                                    sWindowContentColors[1][2], this->nameBoxAlpha[i]);
                } else {
        ...
        for (fileIndex = 0; fileIndex < 3; fileIndex++) {
            ...
            if (Save_GetSaveMetaInfo(fileIndex)->valid && Save_GetSaveMetaInfo(fileIndex)->archipelagoSave) {
                char apSaveLabel[] = "ARCHIPELAGO";
                // File buttons are positioned in the file-select object coordinate
                // system around screen center. Convert their current animated Y to
                // screen coordinates so the badge follows copy/select transitions.
                s16 fileObjectY = this->buttonYOffsets[fileIndex] + (0x2C - (fileIndex * 0x10)) +
                                  this->fileNamesY[fileIndex];
                s16 labelY = (SCREEN_HEIGHT / 2) - fileObjectY + 2;
                Interface_DrawTextLine(this->state.gfxCtx, apSaveLabel, 205, labelY, 80, 220, 255,
                                       this->nameAlpha[fileIndex], 0.45f, true);
            }
        }
