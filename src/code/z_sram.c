...
void Sram_InitSave(FileChooseContext* fileChooseCtx) {
    ...
    if (currentQuest == QUEST_ARCHIPELAGO) {
        // The AP quest is a file-select pseudo mode. Persist it as Randomizer
        // so all of SoH's randomizer item/check hooks remain active.
        Archipelago_InitSaveFile();
    } else if (currentQuest == QUEST_RANDOMIZER &&
