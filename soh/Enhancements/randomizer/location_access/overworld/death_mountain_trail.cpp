...
void RegionTable_Init_DeathMountainTrail() {
    ...
    areaTable[RR_DEATH_MOUNTAIN_TRAIL] = Region("Death Mountain Trail", SCENE_DEATH_MOUNTAIN_TRAIL, {
        ...
        LOCATION(RC_DMT_BLUE_RUPEE,               	      logic->IsChild && (!ctx->GetOption(RSK_SHUFFLE_ROCK_SOUL) || logic->HasItem(RG_ROCK_SOUL)) && logic->BlastOrSmash()),
        LOCATION(RC_DMT_RED_RUPEE,                	      logic->IsChild && (!ctx->GetOption(RSK_SHUFFLE_ROCK_SOUL) || logic->HasItem(RG_ROCK_SOUL)) && logic->BlastOrSmash()),
    ...
    areaTable[RR_DMT_COW_GROTTO] = Region("DMT Cow Grotto", SCENE_GROTTOS, {}, {
        //Locations
        LOCATION(RC_DMT_COW_GROTTO_COW, (!ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) || logic->HasItem(RG_ANIMAL_SOUL)) &&                logic->CanUse(RG_EPONAS_SONG)),
