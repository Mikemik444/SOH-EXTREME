...
void RegionTable_Init_Kakariko() {
    ...
    areaTable[RR_KAKARIKO_VILLAGE] = Region("Kakariko Village", SCENE_KAKARIKO_VILLAGE, {
        ...
        LOCATION(RC_KAK_ANJU_AS_CHILD,                     logic->IsChild && logic->AtDay && (!ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) || logic->HasItem(RG_ANIMAL_SOUL)) && logic->HasItem(RG_CLIMB) && logic->HasItem(RG_POWER_BRACELET) && logic->CanBreakCrates() && logic->HasItem(RG_SPEAK_HYLIAN)), // RANDOTODO adjust requirements to cucco settings
        ...
        LOCATION(RC_KAK_WONDER_UNDER_CONSTRUCTION,         logic->IsChild &&
                                                           (!ctx->GetOption(RSK_ROCS_FEATHER) || logic->HasItem(RG_ROCS_FEATHER))),
        ...
        ENTRANCE(RR_KAK_IMPAS_LEDGE,          (logic->IsChild && (((logic->AtDay && (!ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) || logic->HasItem(RG_ANIMAL_SOUL)) && logic->HasItem(RG_POWER_BRACELET))) || (logic->BunnyHovers() && logic->CanJumpslash()))) || (logic->IsAdult && ctx->GetTrickOption(RT_VISIBLE_COLLISION))),
    ...
    areaTable[RR_KAK_IMPAS_HOUSE] = Region("Kak Impas House", SCENE_IMPAS_HOUSE, {}, {
        //Locations
        LOCATION(RC_KAK_IMPAS_HOUSE_COW, (!ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) || logic->HasItem(RG_ANIMAL_SOUL)) &&  logic->CanUse(RG_EPONAS_SONG)),
