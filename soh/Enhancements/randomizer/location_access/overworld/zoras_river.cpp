...
void RegionTable_Init_ZoraRiver() {
    ...
    areaTable[RR_ZORAS_RIVER] = Region("Zora River", SCENE_ZORAS_RIVER, {
        ...
        LOCATION(RC_ZR_NEAR_DOMAIN_FREESTANDING_POH,         (logic->IsChild && (!ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) || logic->HasItem(RG_ANIMAL_SOUL)) && logic->HasItem(RG_POWER_BRACELET)) || logic->CanUse(RG_BOOMERANG) || logic->CanUse(RG_HOVER_BOOTS) || (logic->IsAdult && ctx->GetTrickOption(RT_ZR_UPPER))),
        ...
        ENTRANCE(RR_ZR_ATOP_LADDER,      ((logic->IsAdult || logic->HasItem(RG_BRONZE_SCALE) || (logic->IsChild && (!ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) || logic->HasItem(RG_ANIMAL_SOUL)) && logic->HasItem(RG_POWER_BRACELET)) || logic->CanUse(RG_HOVER_BOOTS)) && logic->CanClimbLadder()) || (logic->IsAdult && logic->BeanPlanted(LOGIC_PLANT_ZORAS_RIVER_BEAN))),
        ENTRANCE(RR_ZR_PILLAR,           (logic->IsChild && (!ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) || logic->HasItem(RG_ANIMAL_SOUL)) && logic->HasItem(RG_POWER_BRACELET)) || logic->CanUse(RG_HOVER_BOOTS) || (logic->IsAdult && (ctx->GetTrickOption(RT_ZR_LOWER) || logic->BunnyHood()))),
        ...
        ENTRANCE(RR_ZR_BEHIND_WATERFALL, ctx->GetOption(RSK_SLEEPING_WATERFALL).Is(RO_WATERFALL_OPEN) || AnyAgeTime([]{return logic->CanUse(RG_ZELDAS_LULLABY);}) || (logic->IsChild && (!ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) || logic->HasItem(RG_ANIMAL_SOUL)) && ctx->GetTrickOption(RT_ZR_CUCCO) && logic->HasItem(RG_POWER_BRACELET)) || (logic->IsAdult && logic->CanUse(RG_HOVER_BOOTS) && ctx->GetTrickOption(RT_ZR_HOVERS))),
    });
    ...
    areaTable[RR_ZR_ATOP_LADDER] = Region("ZR Atop Ladder", SCENE_ZORAS_RIVER, {
        ...
        ENTRANCE(RR_ZR_PILLAR,       (logic->IsChild && (!ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) || logic->HasItem(RG_ANIMAL_SOUL)) && logic->HasItem(RG_POWER_BRACELET)) || logic->CanUse(RG_HOVER_BOOTS) || (logic->BunnyHood() && (logic->IsAdult || logic->CanJumpslash()))),
