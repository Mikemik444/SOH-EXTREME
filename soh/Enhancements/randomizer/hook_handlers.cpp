...
void RandomizerOnFlagSetHandler(int16_t flagType, int16_t flag) {
    ...
    // Flag-driven checks (freestanding pickups, grass/object sanity, many NPC/event
    // checks) are the earliest reliable point where we know the location was really
    // collected. Report AP here, before SoH can queue and grant its local placement.
    if (loc != nullptr && !loc->HasObtained() && Archipelago_ShouldHandleCheck(static_cast<int32_t>(rc))) {
        SPDLOG_INFO("[Archipelago] Flag collected RC {}; reporting immediately", static_cast<uint32_t>(rc));
        Archipelago_ReportCheck(static_cast<int32_t>(rc));
...
void RandomizerOnSceneFlagSetHandler(int16_t sceneNum, int16_t flagType, int16_t flag) {
    ...
    auto loc = Rando::Context::GetInstance()->GetItemLocation(rc);
    if (loc != nullptr && !loc->HasObtained() && Archipelago_ShouldHandleCheck(static_cast<int32_t>(rc))) {
        SPDLOG_INFO("[Archipelago] Scene flag collected RC {}; reporting immediately", static_cast<uint32_t>(rc));
        Archipelago_ReportCheck(static_cast<int32_t>(rc));
        loc->SetCheckStatus(RCSHOW_COLLECTED);
        CheckTracker::SpoilAreaFromCheck(rc);
        CheckTracker::RecalculateAllAreaTotals();
...
void RandomizerOnPlayerUpdateForRCQueueHandler() {
    ...
    // In an Archipelago file, the server owns the reward for every mapped check.
    // The local randomizer still drives actors/flags/check tracking, but must not
    // grant its independently-generated local item. Marking the check collected
    // fires OnRandoSetCheckStatus, which reports the AP location; APCpp then sends
    // the real item through ReceivedItems and ArchipelagoClient::ProcessItem.
    if (Archipelago_ShouldHandleCheck(static_cast<int32_t>(rc))) {
        SPDLOG_INFO("[Archipelago] RC {} collected; sending immediately and suppressing local reward",
                    static_cast<uint32_t>(rc));
        // Send before tracker/save bookkeeping so the AP packet goes out on the exact
        // collection frame instead of waiting for the periodic reconciliation pass.
        Archipelago_ReportCheck(static_cast<int32_t>(rc));
        loc->SetCheckStatus(RCSHOW_COLLECTED);
        CheckTracker::SpoilAreaFromCheck(rc);
        CheckTracker::RecalculateAllAreaTotals();
    ...
    switch (id) {
        ...
        case VB_GIVE_ITEM_FROM_ITEM_00: {
            ...
            if (item00->actor.params == ITEM00_SOH_DUMMY) {
                // ITEM00 freestanding checks know their exact RC at the instant Link touches
                // them. Report AP right here and mark the location first, before setting the
                // RandomizerInf/collectible flag. That prevents the flag hook from ever
                // queueing the old locally-generated reward (the Bridge-Rupee -> Slingshot bug).
                if (item00->randoCheck != RC_UNKNOWN_CHECK &&
                    Archipelago_ShouldHandleCheck(static_cast<int32_t>(item00->randoCheck))) {
                    auto* apLoc = Rando::Context::GetInstance()->GetItemLocation(item00->randoCheck);
                    SPDLOG_INFO("[Archipelago] ITEM00 touched RC {}; reporting on pickup frame", 
                                static_cast<uint32_t>(item00->randoCheck));
                    Archipelago_ReportCheck(static_cast<int32_t>(item00->randoCheck));
                    if (apLoc != nullptr && !apLoc->HasObtained()) {
                        apLoc->SetCheckStatus(RCSHOW_COLLECTED);
                        CheckTracker::SpoilAreaFromCheck(item00->randoCheck);
                        CheckTracker::RecalculateAllAreaTotals();
                        CheckTracker::RecalculateAvailableChecks();
                        SaveManager::Instance->SaveSection(gSaveContext.fileNum, SECTION_ID_TRACKER_DATA, true);
                    }
