...
static bool MegaIsNpcLocationActor(ActorID id) {
    switch (id) {
        case ACTOR_BG_ZG:
        case ACTOR_DEMO_DU:
        case ACTOR_DEMO_EC:
...
static bool MegaIsAnimalLocationActor(ActorID id) {
    switch (id) {
        case ACTOR_EN_COW:
        case ACTOR_EN_NIW:
        case ACTOR_EN_DOG:
...
static bool MegaSoulAllowsLocation(RandomizerCheck rc) {
    auto ctx = Rando::Context::GetInstance();
    // No-logic intentionally ignores progression rules. Both glitchless and glitched
    // logic must still obey SOH-EXTREME's physical actor gates (souls/shovel/etc.).
    ...
    // Gate checks whose owning actor is an NPC. This keeps playthrough
    // generation and the check tracker in sync with the barren-world runtime
    // rule that hides NPC actors until NPC Soul is obtained.
    if (ctx->GetOption(RSK_SHUFFLE_NPC_SOUL) && MegaIsNpcLocationActor(loc->GetActorID()) &&
        !logic->HasItem(RG_NPC_SOUL)) {
        return false;
    }

    // Runtime Animal Soul removes these actors completely (cuccos, frogs,
    // cows, fish, insects, butterflies and horses).  Gate any check whose
    // owning actor is one of them even when its RCType is just STANDARD.
    if (ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) && MegaIsAnimalLocationActor(loc->GetActorID()) &&
        !logic->HasItem(RG_ANIMAL_SOUL)) {
        return false;
    }
    ...
    switch (loc->GetRCType()) {
        ...
        case RCTYPE_POT:
            ...
            // hookshot/longshot, ranged weapons and valid Bombchu routes remain
            // legal exactly as Logic::CanBreakPots() defines them.
        ...
        case RCTYPE_ROCK:
            return logic->CanBreakRocks();
        case RCTYPE_BOULDER:
            // Individual boulder locations carry the exact hammer/explosive rule; this
            // central layer enforces the shared Rock Soul physical gate.
            return !ctx->GetOption(RSK_SHUFFLE_ROCK_SOUL) || logic->HasItem(RG_ROCK_SOUL);
        case RCTYPE_TREE:
        case RCTYPE_NLTREE:
            return logic->CanBonkTrees();
        case RCTYPE_BEEHIVE:
            return !ctx->GetOption(RSK_SHUFFLE_BEEHIVE_SOUL) || logic->HasItem(RG_BEEHIVE_SOUL);
        case RCTYPE_SIGN:
            return !ctx->GetOption(RSK_SHUFFLE_SIGN_SOUL) || logic->HasItem(RG_SIGN_SOUL);
        case RCTYPE_SKULL_TOKEN:
            return !ctx->GetOption(RSK_SHUFFLE_SKULLTULA_SOUL) || logic->HasItem(RG_SKULLTULA_SOUL);
        case RCTYPE_COW:
        case RCTYPE_FISH:
        case RCTYPE_BUTTERFLY_FAIRY:
            return !ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) || logic->HasItem(RG_ANIMAL_SOUL);
        default:
            return true;
    }
}
...
void Region::ApplyTimePass() {
    // SOH-EXTREME: when Flow of Time is shuffled, outdoor time-pass regions
    // must not magically grant the opposite time of day before the item is owned.
