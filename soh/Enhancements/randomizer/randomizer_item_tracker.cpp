...
static WidgetInfo megaSoulsTracking;
...
std::vector<ItemTrackerItem> megaSoulItems = {
    ITEM_TRACKER_RG(RG_ENEMY_SOUL, "ENEMY", 0, DrawItem),
    ITEM_TRACKER_RG(RG_NPC_SOUL, "NPC", 0, DrawItem),
    ITEM_TRACKER_RG(RG_ANIMAL_SOUL, "ANIMAL", 0, DrawItem),
    ITEM_TRACKER_RG(RG_POT_SOUL, "POT", 0, DrawItem),
    ITEM_TRACKER_RG(RG_CRATE_SOUL, "CRATE", 0, DrawItem),
    ITEM_TRACKER_RG(RG_GRASS_SOUL, "GRASS", 0, DrawItem),
    ITEM_TRACKER_RG(RG_ROCK_SOUL, "ROCK", 0, DrawItem),
    ITEM_TRACKER_RG(RG_TREE_SOUL, "TREE", 0, DrawItem),
    ITEM_TRACKER_RG(RG_BEEHIVE_SOUL, "HIVE", 0, DrawItem),
...
static void TrackMegaSouls(std::vector<ItemTrackerItem>* trackList) {
    for (auto soul : megaSoulItems) {
        bool enabled = false;
        switch (soul.id) {
            case RG_ENEMY_SOUL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_ENEMY_SOUL);
                break;
            case RG_NPC_SOUL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_NPC_SOUL);
                break;
            case RG_ANIMAL_SOUL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_ANIMAL_SOUL);
                break;
            case RG_POT_SOUL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_POT_SOUL);
                break;
            case RG_CRATE_SOUL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_CRATE_SOUL);
                break;
            case RG_GRASS_SOUL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_GRASS_SOUL);
                break;
            case RG_ROCK_SOUL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_ROCK_SOUL);
                break;
            case RG_TREE_SOUL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_TREE_SOUL);
                break;
            case RG_BEEHIVE_SOUL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_BEEHIVE_SOUL);
                break;
            case RG_SIGN_SOUL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_SIGN_SOUL);
                break;
            case RG_SKULLTULA_SOUL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_SKULLTULA_SOUL);
                break;
            case RG_BUSINESS_SCRUB_SOUL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_BUSINESS_SCRUB_SOUL);
                break;
            case RG_SHOVEL:
                enabled = IS_RANDO && RAND_GET_OPTION(RSK_SHUFFLE_SHOVEL);
                break;
            default:
                break;
        }
        if (enabled) {
            trackList->push_back(soul);
        }
    }
}
    ...
    if (item.kind == ITEM_KIND_ITEM) {
        ...
        switch (item.id) {
            ...
            case RG_TREE_SOUL:
                ...
                break;
            case RG_BEEHIVE_SOUL:
                itemName = "Beehive Soul";
                break;
            case RG_SIGN_SOUL:
                itemName = "Sign Soul";
                break;
            case RG_SKULLTULA_SOUL:
                itemName = "Skulltula Soul";
                break;
            case RG_BUSINESS_SCRUB_SOUL:
                itemName = "Scrub Soul";
                break;
...
static int32_t MegaSoulsDisplay() {
    const bool anyMegaSoul =
        IS_RANDO && (RAND_GET_OPTION(RSK_SHUFFLE_ENEMY_SOUL) || RAND_GET_OPTION(RSK_SHUFFLE_NPC_SOUL) ||
                     RAND_GET_OPTION(RSK_SHUFFLE_ANIMAL_SOUL) || RAND_GET_OPTION(RSK_SHUFFLE_POT_SOUL) ||
                     RAND_GET_OPTION(RSK_SHUFFLE_CRATE_SOUL) || RAND_GET_OPTION(RSK_SHUFFLE_GRASS_SOUL) ||
                     RAND_GET_OPTION(RSK_SHUFFLE_ROCK_SOUL) || RAND_GET_OPTION(RSK_SHUFFLE_TREE_SOUL) ||
                     RAND_GET_OPTION(RSK_SHUFFLE_BEEHIVE_SOUL) || RAND_GET_OPTION(RSK_SHUFFLE_SIGN_SOUL) ||
                     RAND_GET_OPTION(RSK_SHUFFLE_SKULLTULA_SOUL) || RAND_GET_OPTION(RSK_SHUFFLE_SHOVEL));
    return GetGatedSectionDisplay(CVAR_TRACKER_ITEM("DisplayType.MegaSouls"), anyMegaSoul);
}
...
void UpdateVectors() {
    ...
    // If we're adding Mega category souls to the main window...
    if (MegaSoulsDisplay() == SECTION_DISPLAY_MAIN_WINDOW) {
        while (mainWindowItems.size() % 6) {
            mainWindowItems.push_back(ITEM_TRACKER_ITEM(ITEM_NONE, "", 0, DrawItem));
        }
        TrackMegaSouls(&mainWindowItems);
    }
...
static bool SongNoteTrackingActive() {
    if (!IS_RANDO) {
        return false;
    }

    if (RAND_GET_OPTION(RSK_SONG_NOTE_SHUFFLE)) {
        return true;
    }

    // AP saves can receive note flags before the native setting cache has been
    // refreshed. If any individual note exists, keep the tracker visible.
    for (int i = 0; i < 74; ++i) {
        if (Flags_GetRandomizerInf(static_cast<RandomizerInf>(
                static_cast<int>(RAND_INF_SONG_NOTE_0) + i))) {
            return true;
        }
    }
...
static void DrawSongNoteProgress() {
    if (!SongNoteTrackingActive()) {
        return;
    }
    ...
    static constexpr SongNoteRow songs[] = {
        { "Zelda's Lullaby", 6, { "C<", "C^", "C>", "C<", "C^", "C>" } },
        { "Epona's Song", 6, { "C^", "C<", "C>", "C^", "C<", "C>" } },
        { "Saria's Song", 6, { "Cv", "C>", "C<", "Cv", "C>", "C<" } },
        { "Sun's Song", 6, { "C>", "Cv", "C^", "C>", "Cv", "C^" } },
        { "Song of Time", 6, { "C>", "A", "Cv", "C>", "A", "Cv" } },
        { "Song of Storms", 6, { "A", "Cv", "C^", "A", "Cv", "C^" } },
        { "Minuet of Forest", 6, { "A", "C^", "C<", "C>", "C<", "C>" } },
        { "Bolero of Fire", 8, { "Cv", "A", "Cv", "A", "C>", "Cv", "C>", "Cv" } },
        { "Serenade of Water", 5, { "A", "Cv", "C>", "C>", "C<" } },
        { "Requiem of Spirit", 6, { "A", "Cv", "A", "C>", "Cv", "A" } },
        { "Nocturne of Shadow", 7, { "C<", "C>", "C>", "A", "C<", "C>", "Cv" } },
        { "Prelude of Light", 6, { "C^", "C>", "C^", "C>", "C<", "C^" } },
    };

    ImGui::SeparatorText("Song Notes");
    ...
    for (const auto& song : songs) {
        ...
        ImGui::Text("%s  %d/%d", song.name, haveCount, song.count);
        ...
        for (int i = 0; i < song.count; ++i) {
            ...
            const bool have = Flags_GetRandomizerInf(static_cast<RandomizerInf>(
                static_cast<int>(RAND_INF_SONG_NOTE_0) + offset + i));

            if (have) {
                ImGui::Text("[%s]", song.sequence[i]);
            } else {
                ImGui::TextDisabled("[%s]", song.sequence[i]);
            }
        }
    ...
    ImGui::Separator();
    ImGui::Text("Total Song Notes: %d / 74", totalHave);
}
...
void ItemTrackerWindow::DrawElement() {
    ...
    if (CVarGetInteger(CVAR_TRACKER_ITEM("WindowType"), TRACKER_WINDOW_FLOATING) == TRACKER_WINDOW_WINDOW ||
        ...
        if ((CVarGetInteger(CVAR_TRACKER_ITEM("DisplayType.Inventory"), SECTION_DISPLAY_MAIN_WINDOW) ==
            ...
            // Individual note progress is its own tracker section. It should remain
            // visible even when the normal Songs icon section is hidden or separated.
            DrawSongNoteProgress();
        ...
        if (MegaSoulsDisplay() == SECTION_DISPLAY_SEPARATE) {
            BeginFloatingWindows("Mega Soul Tracker");
            std::vector<ItemTrackerItem> shuffledMegaSoulItems;
            TrackMegaSouls(&shuffledMegaSoulItems);
            DrawItemsInRows(shuffledMegaSoulItems);
            Trackers::EndFloatWindows();
        }
...
void ItemTrackerSettingsWindow::DrawElement() {
    ...
    if (ImGui::BeginTable("itemTrackerSettingsTable", 2, ImGuiTableFlags_BordersH | ImGuiTableFlags_BordersV)) {
        ...
        SohGui::mSohMenu->MenuDrawItem(megaSoulsTracking, 250, THEME_COLOR);
...
void RegisterItemTrackerWidgets() {
    ...
    megaSoulsTracking = { .name = "Mega Progression", .type = WidgetType::WIDGET_CVAR_COMBOBOX };
    megaSoulsTracking.CVar(CVAR_TRACKER_ITEM("DisplayType.MegaSouls"))
        .Options(ComboboxOptions()
                     .DefaultIndex(SECTION_DISPLAY_MAIN_WINDOW)
                     .ComponentAlignment(ComponentAlignments::Right)
                     .LabelPosition(LabelPositions::Far)
                     ...
                     .ComboMap(displayTypes))
        .Callback([](WidgetInfo& info) { RefreshItemTrackerMainWindow(); });
    SohGui::mSohMenu->AddSearchWidget({ megaSoulsTracking, "Randomizer", "Item Tracker", "General Settings", "icon" });
