...
GetItemEntry Context::GetFinalGIEntry(const RandomizerCheck rc, const bool checkObtainability, const GetItemID ogItemId,
                                      bool spoilAreas) {
    // AP owns the physical placement in SOH-EXTREME. Refresh the individual
    // location immediately before selecting its GetItemEntry so actors cannot
    // cache/display the native seed's stale model. This intentionally lives here
    // rather than in every actor-specific shuffle implementation.
    ...
    // For AP-owned locations the server owns delivery/progression.  Local
    // obtainability must not replace the physical model with a blue rupee.
    // This was the remaining reason Mido's House and some freestanding/drop
    // actors showed the native fallback even after the AP scout was applied.
    const bool apOwnedCheck = Archipelago_ShouldHandleCheck(static_cast<int32_t>(rc));
    if (checkObtainability && !apOwnedCheck &&
        OTRGlobals::Instance->gRandomizer->GetItemObtainabilityFromRandomizerGet(
