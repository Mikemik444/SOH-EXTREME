...
extern "C" u16 Randomizer_Item_Give(PlayState* play, GetItemEntry giEntry) {
    ...
    switch (item) {
        ...
        case RG_DOUBLE_DEFENSE:
            ...
            break;
        case RG_GOLD_SKULLTULA_TOKEN:
            // AP/network rewards bypass vanilla Item_Give for randomizer items,
            // so reproduce ITEM_SKULL_TOKEN here. This updates both the counter
            // and the quest bit used by the Start menu.
            ...
            break;
        case RG_PIECE_OF_HEART: {
            // Randomizer heart pieces are non-major, so AP applies them directly
            // through Randomizer_Item_Give instead of vanilla Item_Give. Mirror
            // vanilla's heart-piece nibble/stat update and immediately perform
            // the fourth-piece conversion normally handled after a get-item.
            gSaveContext.inventory.questItems += 1 << (QUEST_HEART_PIECE + 4);
            gSaveContext.ship.stats.heartPieces++;

            s32 heartPieces =
                (s32)(gSaveContext.inventory.questItems & 0xF0000000) >> (QUEST_HEART_PIECE + 4);
            if (heartPieces >= 4) {
                gSaveContext.inventory.questItems &= ~0xF0000000;
                gSaveContext.inventory.questItems += (heartPieces % 4) << (QUEST_HEART_PIECE + 4);
                const s16 gainedHealth = FULL_HEART_HEALTH * (heartPieces / 4);
                gSaveContext.healthCapacity += gainedHealth;
                gSaveContext.health += gainedHealth;
            }
            gSaveContext.healthAccumulator = MAX_HEALTH;
            break;
        }
        case RG_HEART_CONTAINER:
            // Same reason as Heart Pieces: AP's non-major fast path bypasses
            // vanilla Item_Give, so apply the vanilla container effect here.
        ...
        case RG_GANONS_CASTLE_MQ_SILVER_WATER:
        case RG_GANONS_CASTLE_MQ_SILVER_SHADOW: {
            s8* field = Randomizer::SilverFieldFromSaveContext(&gSaveContext, item);
        ...
        default:
            ...
            // item instead of discarding/asserting merely because this switch
            // has no bespoke randomizer case.
            if (giEntry.itemId != ITEM_NONE) {
                SPDLOG_INFO("Randomizer_Item_Give falling back to vanilla Item_Give for getItemId={} itemId={}",
                            item, giEntry.itemId);
                Item_Give(play, static_cast<uint8_t>(giEntry.itemId));
                break;
            }

            // A true randomizer-only item reaching here still needs an explicit
            // handler, but don't crash or pretend it was applied.
            SPDLOG_ERROR("Randomizer_Item_Give has no behaviour or vanilla fallback for getItemId={}", item);
            return -1;
    }
