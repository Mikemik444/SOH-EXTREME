...
bool Logic::HasItem(RandomizerGet itemName) {
    switch (itemName) {
        ...
        // SOH-EXTREME custom progression items are RandomizerInf-backed rather
        // than vanilla inventory slots. These must be handled explicitly or the
        // native check tracker treats them as missing forever.
        case RG_FLOW_OF_TIME:
            return CheckRandoInf(RAND_INF_FLOW_OF_TIME);
        case RG_ROCS_FEATHER:
            return CheckRandoInf(RAND_INF_OBTAINED_ROCS_FEATHER);
        ...
        case RG_SPEAK_ZORA:
            if (ctx->GetOption(RSK_SHUFFLE_NPC_SOUL) && !CheckRandoInf(RAND_INF_NPC_SOUL)) {
                return false;
            }
            return CheckRandoInf(StaticData::RandoGetToRandInf.at(itemName));
            // Ocarina Buttons
        ...
        // Mega Randomizer persistent progression items
        case RG_ROLL:
        case RG_ENEMY_SOUL:
        case RG_NPC_SOUL:
        case RG_ANIMAL_SOUL:
...
bool Logic::CanKillEnemy(RandomizerEnemy enemy, EnemyDistance distance, bool wallOrFloor, uint8_t quantity, bool timer,
                         bool inWater) {
    if (enemy == RE_GOLD_SKULLTULA && ctx->GetOption(RSK_SHUFFLE_SKULLTULA_SOUL) && !HasItem(RG_SKULLTULA_SOUL)) {
        return false;
    }
...
bool Logic::CanBreakPots(EnemyDistance distance, bool wallOrFloor, bool inWater) {
    if (ctx->GetOption(RSK_SHUFFLE_POT_SOUL) && !HasItem(RG_POT_SOUL)) { return false; }
    bool hit = false;
    switch (distance) {
        case ED_CLOSE:
...
bool Logic::CanBreakCrates() {
    return !ctx->GetOption(RSK_SHUFFLE_CRATE_SOUL) || HasItem(RG_CRATE_SOUL);
}

bool Logic::CanBreakSmallCrates() {
    if (ctx->GetOption(RSK_SHUFFLE_CRATE_SOUL) && !HasItem(RG_CRATE_SOUL)) { return false; }
    return CanJumpslash() || HasExplosives() || HasItem(RG_POWER_BRACELET);
}

bool Logic::CanBreakRocks() {
    if (ctx->GetOption(RSK_SHUFFLE_ROCK_SOUL) && !HasItem(RG_ROCK_SOUL)) { return false; }
    return BlastOrSmash() || HasItem(RG_POWER_BRACELET);
}

bool Logic::CanBonkTrees() {
    if (ctx->GetOption(RSK_SHUFFLE_TREE_SOUL) && !HasItem(RG_TREE_SOUL)) { return false; }
    if (ctx->GetOption(RSK_SHUFFLE_ROLL) && !HasItem(RG_ROLL)) { return false; }
    return true;
}

bool Logic::CanRead() {
    return !ctx->GetOption(RSK_SHUFFLE_SIGN_SOUL) || HasItem(RG_SIGN_SOUL);
}
...
bool Logic::CanOpenBombGrotto() {
    // Shovel shuffle is a physical world gate in SOH-EXTREME: hidden grottos do not
    // exist until the shovel has been received. Keep native SoH logic identical to
    // the runtime actor rule so generation/tracker cannot route through one early.
    if (ctx->GetOption(RSK_SHUFFLE_SHOVEL) && !HasItem(RG_SHOVEL)) {
        return false;
    }
...
bool Logic::CanBreakUpperBeehives() {
    if (ctx->GetOption(RSK_SHUFFLE_BEEHIVE_SOUL) && !HasItem(RG_BEEHIVE_SOUL)) { return false; }
...
std::map<RandomizerGet, uint32_t> StaticData::RandoGetToRandInf = {
    ...
    { RG_ROLL, RAND_INF_HAS_ROLL },
    { RG_ENEMY_SOUL, RAND_INF_ENEMY_SOUL },
    { RG_NPC_SOUL, RAND_INF_NPC_SOUL },
    { RG_ANIMAL_SOUL, RAND_INF_ANIMAL_SOUL },
    { RG_POT_SOUL, RAND_INF_POT_SOUL },
    { RG_CRATE_SOUL, RAND_INF_CRATE_SOUL },
    { RG_GRASS_SOUL, RAND_INF_GRASS_SOUL },
    { RG_ROCK_SOUL, RAND_INF_ROCK_SOUL },
    { RG_TREE_SOUL, RAND_INF_TREE_SOUL },
    { RG_BEEHIVE_SOUL, RAND_INF_BEEHIVE_SOUL },
    { RG_SIGN_SOUL, RAND_INF_SIGN_SOUL },
    { RG_SKULLTULA_SOUL, RAND_INF_SKULLTULA_SOUL },
    { RG_BUSINESS_SCRUB_SOUL, RAND_INF_BUSINESS_SCRUB_SOUL },
    { RG_SHOVEL, RAND_INF_HAS_SHOVEL },
    ...
    { RG_FLOW_OF_TIME, RAND_INF_FLOW_OF_TIME },
...
bool Logic::SummonEpona() {
    ...
    const bool animalAvailable = !ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) || HasItem(RG_ANIMAL_SOUL);
    return animalAvailable && IsAdult && Get(LOGIC_FREED_EPONA) && CanUse(RG_EPONAS_SONG);
}
