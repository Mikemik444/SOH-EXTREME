...
std::vector<RandomizerCheck> ReachabilitySearch(const std::vector<RandomizerCheck>& targetLocations,
    ...
    if (startingRegion != RR_ROOT) {
        ...
        // SOH-EXTREME: seeding the check tracker from the current region used to
        // grant the opposite time immediately whenever the scene normally passes
        // time. That bypassed Region::ApplyTimePass() and therefore bypassed the
