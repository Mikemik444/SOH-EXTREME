...
static void RollRandomTrap(uint64_t seed) {
    ...
    switch (roll) {
        ...
        case ADD_MAGIC_TRAP:
            ...
            Notification::Emit({ .message = "Magic Drained!" });
            break;
        case ADD_HEALTH_TRAP:
            GameInteractor::RawAction::HealOrDamagePlayer(-2);
            Notification::Emit({ .message = "Health Drained!" });
            break;
        default:
            break;
    }
