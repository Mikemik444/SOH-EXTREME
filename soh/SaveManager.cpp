...
SaveManager::SaveManager() {
    ...
    for (SaveFileMetaInfo& info : fileMetaInfo) {
        ...
        info.randoSave = 0;
        info.archipelagoSave = false;
        memset(info.archipelagoSlot, 0, sizeof(info.archipelagoSlot));
...
void SaveManager::LoadRandomizer() {
    ...
    // SOH-EXTREME Archipelago persistence.  These values live in the randomizer
    // save section, so receipt progress follows the individual save instead of a
    // global CVar shared by every file using the same slot.
    bool archipelagoSave = false;
    ...
    SaveManager::Instance->LoadData("archipelagoFallbackNpcSpeechCount", archipelagoFallbackNpcSpeechCount, (size_t)0);
    SaveManager::Instance->LoadArray("archipelagoFallbackNpcSpeechHashes", archipelagoFallbackNpcSpeechCount,
                                     [&](size_t i) {
                                         uint64_t hash = 0;
                                         SaveManager::Instance->LoadData("", hash, (uint64_t)0);
                                         if (hash != 0) archipelagoFallbackNpcSpeechHashes.push_back(hash);
                                     });
...
void SaveManager::SaveRandomizer(SaveContext* saveContext, int sectionID, bool fullSave) {
    ...
    if (archipelago.IsCurrentSaveArchipelago()) {
        ...
        SaveManager::Instance->SaveArray("archipelagoFallbackNpcSpeechHashes", fallbackNpcSpeechHashes.size(),
                                         [&](size_t i) {
                                             SaveManager::Instance->SaveData("", fallbackNpcSpeechHashes[i]);
                                         });
    }
...
void SaveManager::StartupCheckAndInitMeta(int fileNum) {
    ...
    try {
        ...
        fileMetaInfo[fileNum].archipelagoSave = false;
        memset(fileMetaInfo[fileNum].archipelagoSlot, 0, sizeof(fileMetaInfo[fileNum].archipelagoSlot));
        if (isRando) {
...
void SaveManager::InitMeta(int fileNum) {
    ...
    fileMetaInfo[fileNum].archipelagoSave = IS_RANDO && apClient.IsCurrentSaveArchipelago();
    memset(fileMetaInfo[fileNum].archipelagoSlot, 0, sizeof(fileMetaInfo[fileNum].archipelagoSlot));
    if (fileMetaInfo[fileNum].archipelagoSave) {
        SohUtils::CopyStringToCharArray(fileMetaInfo[fileNum].archipelagoSlot, apClient.GetSaveSlot(),
                                        ARRAY_COUNT(fileMetaInfo[fileNum].archipelagoSlot));
    }
